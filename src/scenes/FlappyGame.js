...
class FlappyGame extends Phaser.Scene {
    constructor() {
        super({
            key: 'FlappyGame',
            backgroundImage: require('../assets/mini_game/mini-game_bg.jpg'),
            physics: {
                default: 'arcade',
                arcade: {
                    x: 0,
    ...
    preload() {
        this.load.image('FlappyGame:head', require('../assets/mini_game/head.png'))
        this.load.image('FlappyGame:pipe', require('../assets/mini_game/pipe.png'))
        this.load.image(
            'FlappyGame:background',
            require('../assets/mini_game/mini-game_bg.jpg')
        )
    }
    create() {
        // FIXME: add dpi handler
        this.add
            .image(0, 0, 'FlappyGame:background')
            .setScale(1 * window.devicePixelRatio, 1 * window.devicePixelRatio)
            .setOrigin(0, 0)
        ...
        this.bird = this.physics.add
            .sprite(80, this.sys.game.config.height / 2, 'FlappyGame:head')
            .setScale(1 * window.devicePixelRatio, 1 * window.devicePixelRatio)
